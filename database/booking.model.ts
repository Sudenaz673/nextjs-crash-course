...
// DÜZELTME BURADA: (next) yerine (next: any) yazdık
BookingSchema.pre('save', async function() {
    ...
    if (booking.isModified('eventId') || booking.isNew) {
        try {
            ...
            if (!eventExists) {
                // next(error) yerine direkt hata fırlatıyoruz
                const error = new Error(`Event with ID ${booking.eventId} does not exist`);
                error.name = 'ValidationError';
                throw error;
            }
        } catch (error) {
            // Eğer yukarıdaki hata bizden değilse genel hata fırlat
